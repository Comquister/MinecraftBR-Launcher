...
try {
    ...
    if (!$exeAsset) {throw "Nenhum arquivo .exe encontrado no release"}
    $localPath = Join-Path $downloadDir $exeAsset.name
    $downloadUrl = $exeAsset.browser_download_url
    $currentHash = $null
    if (Test-Path $localPath) {$currentHash = (Get-FileHash $localPath -Algorithm SHA256).Hash}
    ...
    if ($currentHash -ne $newHash) {Write-Output "✅ Launcher atualizado com sucesso!"} else {Write-Output "ℹ️  Launcher já está na versão mais recente"}
